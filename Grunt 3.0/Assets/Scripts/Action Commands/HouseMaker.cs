...
public class HouseMaker : MonoBehaviour {
	...
	void OnDestroy()
	{
		BattleManager.self.CurrentAttackState = AttackStateEnum.MovePostAction;
		...
		foreach(BattleCharacter bcIter in BattleManager.self.TargetFriendlies)
		{
			House effect = Instantiate (Engine.self.statusEffectPrefab).AddComponent<House> ();
