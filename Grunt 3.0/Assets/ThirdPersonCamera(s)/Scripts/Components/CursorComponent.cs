...
namespace AdvancedUtilities.Cameras.Components
{
    ...
    public class CursorComponent : CameraComponent
    {
        ...
        public void SetCursorLock()
        {
            ...
            if (LockCursorButton)
            {
                _cursorLocked = Input.GetButton(LockButtonInputName);
            }
        ...
        /// If the cursor is set to lock when the lock button is pressed, then this will be overridden.
        /// This will also be overriden if the cursor is set to unlock when the release lock button is pressed.
        /// </summary>
        /// <param name="value">Whether you want the cursor to be locked or not.</param>
